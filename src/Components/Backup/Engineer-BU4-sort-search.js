...
class Engineer extends Component {
  ...
  clickHandler(event) {
    ...
    axios
      .get(this.state.f_url + and + querySearch)
      .then((item) => {
        this.setState({
          engineerses: item.data.response,
          f_url2: url + querySearch,
        });
      })
  ...
  changePagination(pageNumber) {
    ...
    axios
      .get(page_url + and + querySearch + offset)
      .then((item) => {
  ...
  getDataFromApi() {
    axios
      ...
      .then((item) => {
        this.setState({
          engineerses: item.data.response,
          total: item.data.response.length,
        });
      })
      .catch((err) => console.log(err));
  }
