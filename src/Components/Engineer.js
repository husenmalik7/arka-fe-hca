...
import {  Button, InputGroup, ButtonGroup, Dropdown, DropdownButton, Row,
          FormControl, Form, Navbar, CardDeck, Card, CardColumns, Nav, Pagination, Col
...
class Engineer extends Component{
  ...
  submitSearch = async () => {
    ...
    console.log(URL+'?'+this.state.query_searchBox+this.state.query_sortButton+this.state.query_pagin);
    await this.props.dispatch( getAllEngineer(URL+'?'+this.state.query_searchBox+this.state.query_sortButton+this.state.query_pagin) );
    this.setState({
   ...
   {engineer.map((item) => (
    <Card key={item.id_engineer}>
      ...
      <Card.Body>
        ...
        <Card.Text> <span className="glyphicon">&#x2710; </span> {item.total_project} </Card.Text>
        <Card.Text> <span className="glyphicon">&#x2709; </span> { (item.total_project_done / item.total_project) * 100 } % </Card.Text>
      </Card.Body>
      <Card.Footer align="center">

      <Link to={`/engineer/${item.id_engineer}`}>
        <Button variant="dark" id={item.id_engineer} onClick={this.hire}>HIRE</Button>
      </Link>
      </Card.Footer>
    </Card> 
  ))}
